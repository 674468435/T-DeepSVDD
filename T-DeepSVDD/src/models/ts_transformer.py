...
def model_factory(config):
    ...
    if (task == "imputation") or (task == "transduction") or (task == "reconstruction"):
        if config['model'] == 'LINEAR': pass
    ...
    if (task == "classification") or (task == "regression") or (task == "localization"):
...
class TransformerBatchNormEncoderLayer(nn.modules.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.linear1 = Linear(d_model, dim_feedforward, bias=False)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model, bias=False)
...
class TSTransformerEncoder(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, dropout=0.1,
        ...
        self.project_inp = nn.Linear(feat_dim, d_model, bias=False)
        ...
        self.encoder_layer_1 = nn.Linear(d_model * max_len, 1024, bias=False)
        self.encoder_layer_2 = nn.Linear(1024, 512, bias=False)
        self.encoder_layer_3 = nn.Linear(512, 128, bias=False)
        # self.decoder_layer = nn.Linear(64, 128, bias=False)
        #
        # self.output_layer = nn.Linear(128, self.max_len, bias=False)
    ...
    def forward(self, X, padding_masks):
        ...
        output = self.encoder_layer_1(output); # (batch_size, 1024)
        output = self.encoder_layer_2(output); # (batch_size, 512)
        output = self.encoder_layer_3(output); # (batch_size, 128)
