...
from datasets.dataset import ImputationDataset, TransductionDataset, ClassiregressionDataset, collate_unsuperv, collate_superv, collate_recons
...
def pipeline_factory(config):
    ...
    if task == 'reconstruction':
        return partial(ImputationDataset, mean_mask_length=config['mean_mask_length'],
                       masking_ratio=config['masking_ratio'], mode=config['mask_mode'],
                       distribution=config['mask_distribution'], exclude_feats=config['exclude_feats']),\
                        collate_recons, UnsupervisedRunner
...
class BaseRunner(object):
    ...
    def evaluate(self, epoch_num=None, keep_all=True, config = None):
        raise NotImplementedError('Please override in child class')
...
class UnsupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None, config=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            X, targets, target_masks, padding_masks, IDs, trends = batch
